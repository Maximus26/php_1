<?php
/**
 * функция генерации меню из массива
 * @param array
 * @return string
 */
function menu($arr){
	...
	foreach($arr as $value){
		if(isset($value['children'])){
			$child = menu($value['children']);
		}
		$item .= '<li><a href="' . $value['link'] . '">' . $value['title'] . '</a>' . $child . '</li>';
	}
	$items = '<ul>' . $item . '</ul>';
	return $items;
}
 ...
 * @param $fileName - имя name заданное для input типа file
 * @param $path папка куда будет помещен файл
 * @return string
 */
 ...
 * @return bool
 */
function registration($name, $login, $password, $role = 0) {

	$db = createConnection();
	$name = escapeString($db, $name);
	$login = escapeString($db, $login);
	$password = escapeString($db, $password);
	$sql = "INSERT INTO `users`(`name`, `login`, `password`, `role`) VALUES ('$name', '$login', '$password', '$role')";
	return execQuery($sql, $db);
}

/**
 * Функция берет массив и выводит новый в качестве ключа берет указанный ключь исходного
 * @param array $array
 * @param string $key
 * @return array
 */
